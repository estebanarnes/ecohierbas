...
const Productos = () => {
  ...
  const {
    data: page,
    template
  } = useWordPressTemplate('productos');
  const {
    useProducts,
    useCategories
  } = useWooCommerce();
  ...
  const allProducts = wcProducts ? wcProducts.map(wcProduct => ({
    id: wcProduct.id,
    name: wcProduct.name,
    slug: wcProduct.slug,
  ...
  return <PageTemplate page={page} template={template} customSEO={{
        ...
        <div className="space-y-8">
          ...
          return <div className="relative">
                    <div className="mb-6 flex justify-between items-center">
                      ...
                      {pages.length > 1 && <div className="text-sm text-muted-foreground">
              ...
              loop: true
                      ...
                      <CarouselContent>
                        {pages.map((pageProducts, pageIndex) => <CarouselItem key={pageIndex} className="pl-4">
                    ...
                    {pages.length > 1 && <div className="flex justify-center mt-6">
                        <div className="flex gap-2">
                          {pages.map((_, index) => <div key={index} className="w-2 h-2 rounded-full bg-muted"></div>)}
                        </div>
                      </div>}
                  </div>;
        })()}
